...
// nav2_amcl 1.3.11's include/nav2_amcl/amcl_node.hpp
// (https://github.com/ros-navigation/navigation2/blob/1.3.11/nav2_amcl/include/nav2_amcl/amcl_node.hpp),
// confirmed byte-identical to the header actually shipped in the installed
// ros-jazzy-nav2-amcl 1.3.11-1noble.20260412.054619 package before any edit
// below was made. The only changes from that upstream file are marked
// "C2-NAV.36" and are: (1) this banner, (2) the include guard and
// enclosing namespace renamed from nav2_amcl to coco_nav_diag so this
// class cannot collide with the installed nav2_amcl::AmclNode symbol, and
// (3) two new private members (a DiagRecorder and an update-index counter)
// used by the instrumentation in amcl_node.cpp. No public behavior,
// parameter, topic, or algorithm is changed. See docs/agents/HANDOFF.md
// for the full instrumentation point and schema.
// ---------------------------------------------------------------------

#ifndef COCO_NAV_DIAG__AMCL_NODE_HPP_
#define COCO_NAV_DIAG__AMCL_NODE_HPP_
...
protected:
  ...
  // C2-NAV.36: added members. diag_recorder_ is default-constructed
  // disabled (see DiagRecorder); configure() is called once from
  // initParameters() using the diag_* parameters declared in the
  // constructor. diag_update_index_ counts laserReceived() invocations
  // that reach the getOdomPose() call, and is read directly by both
  // getOdomPose() and laserReceived() (same object, no signature change
  // needed on either).
  coco_nav_diag::DiagRecorder diag_recorder_;
  uint64_t diag_update_index_{0};
  // update_index of the laserReceived() cycle that last set pf_odom_pose_
  // (0 = none yet). Updated at both of pf_odom_pose_'s two mutation sites
  // (the anchor-init branch, and the end of updateFilter()) so
  // MotionDeltaEvent::anchor_update_index always names the real producer.
  uint64_t diag_anchor_update_index_{0};
...
#endif  // COCO_NAV_DIAG__AMCL_NODE_HPP_
