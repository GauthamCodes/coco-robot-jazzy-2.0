...
// ---------------------------------------------------------------------
// C2-NAV.36 (coco-robot-ros2): this file is a version-matched fork of
// nav2_amcl 1.3.11's src/amcl_node.cpp
// (https://github.com/ros-navigation/navigation2/blob/1.3.11/nav2_amcl/src/amcl_node.cpp).
// The companion header (coco_nav_diag/amcl_node.hpp) was verified
// byte-identical to the header actually shipped in the installed
// ros-jazzy-nav2-amcl 1.3.11-1noble.20260412.054619 package; this .cpp
// could not be verified the same way because no .cpp is installed on this
// machine (headers and compiled .so only) -- see docs/agents/HANDOFF.md
// for what that leaves unverified.
//
// Every change from the fetched upstream tag is marked "C2-NAV.36" and is
// limited to: this banner, the namespace rename (nav2_amcl -> coco_nav_diag,
// to avoid colliding with the installed nav2_amcl::AmclNode symbol), three
// new default-off diag_* parameters, and two instrumentation call sites
// (the getOdomPose() TF lookup, and the laserReceived() anchor/motion-delta
// computation) that record to DiagRecorder when diag_enabled=true and are a
// single disabled-check branch otherwise. No parameter default, topic,
// frame, threshold, or algorithm step is changed; a diff against the
// fetched upstream file (`git diff --no-index`) is expected to show only
// these additions, never a deletion or reorder of existing logic.
// ---------------------------------------------------------------------
...
// C2-NAV.36: namespace renamed from nav2_amcl to coco_nav_diag.
namespace coco_nav_diag
{
using nav2_util::geometry_utils::orientationAroundZAxis;
// C2-NAV.36: needed because this file's enclosing namespace is no longer
// nav2_amcl (see banner above). Every angleutils::angle_diff() call below
// -- both pre-existing (shouldUpdateFilter()) and new (the C2-NAV.36
// motion-delta event) -- resolves through this to nav2_amcl::angleutils,
// the same class the still-installed nav2_amcl::DifferentialMotionModel
// uses.
using nav2_amcl::angleutils;

namespace
{
// C2-NAV.36: split an rclcpp::Time into (sec, nanosec) for the diagnostic
// recorder, which stays free of rclcpp/builtin_interfaces types so it can
// be unit-tested without a ROS runtime.
void diagSplitStamp(const rclcpp::Time & t, int32_t & sec, uint32_t & nanosec)
{
  const int64_t ns = t.nanoseconds();
  sec = static_cast<int32_t>(ns / 1000000000LL);
  nanosec = static_cast<uint32_t>(ns - static_cast<int64_t>(sec) * 1000000000LL);
}
}  // namespace
...
: nav2_util::LifecycleNode("amcl", "", options)
{
  ...
  // C2-NAV.36: added parameters. Default-disabled diagnostic capture of
  // the exact scan-stamped odom TF lookup and motion-delta AMCL consumes.
  // See DiagRecorder (coco_nav_diag/diag_recorder.hpp) and
  // docs/agents/HANDOFF.md for the schema and how to enable this.
  add_parameter(
    "diag_enabled", rclcpp::ParameterValue(false),
    "C2-NAV.36 diagnostic: when true, records the exact odom TF lookup and "
    "motion-model delta this node consumes to diag_output_path as bounded "
    "JSONL. Default false. Does not change AMCL behavior, only observes it.");

  add_parameter(
    "diag_output_path", rclcpp::ParameterValue(std::string("")),
    "C2-NAV.36 diagnostic: output path for the JSONL capture. Empty (the "
    "default) means capture stays disabled even if diag_enabled is true "
    "(fail-safe: enabling capture with no destination is a no-op, not an "
    "error).");

  add_parameter(
    "diag_max_events", rclcpp::ParameterValue(200000),
    "C2-NAV.36 diagnostic: bound on buffered-but-not-yet-flushed diagnostic "
    "events. Beyond this, newer events are dropped and counted rather than "
    "growing memory unboundedly; see the diag_events_dropped log at "
    "shutdown.");
}

AmclNode::~AmclNode()
{
  // C2-NAV.36: surface dropped-event count, if any, before diag_recorder_
  // (a member, destroyed after this body returns) flushes and closes.
  if (diag_recorder_.enabled() && diag_recorder_.droppedCount() > 0) {
    RCLCPP_WARN(
      get_logger(),
      "C2-NAV.36 diagnostic capture dropped %lu events (diag_max_events reached)",
      static_cast<unsigned long>(diag_recorder_.droppedCount()));
  }
}
  ...
  const rclcpp::Time & sensor_timestamp, const std::string & frame_id)
{
  ...
  try {
    ...
    // C2-NAV.36: record the TF lookup failure at its exact call site. No
    // second/alternate lookup is performed -- this is the same exception
    // from the same tf_buffer_->transform() call laserReceived() depends
    // on for its early return.
    if (diag_recorder_.enabled()) {
      coco_nav_diag::OdomTfLookupEvent ev;
      ev.update_index = diag_update_index_;
      diagSplitStamp(sensor_timestamp, ev.scan_stamp_sec, ev.scan_stamp_nanosec);
      ev.base_frame_id = frame_id;
      ev.odom_frame_id = odom_frame_id_;
      ev.lookup_success = false;
      ev.error_message = e.what();
      ev.consecutive_failures = scan_error_count_;
      diagSplitStamp(now(), ev.node_now_sec, ev.node_now_nanosec);
      diag_recorder_.recordOdomTfLookup(ev);
    }
    return false;
  }
  ...
  // C2-NAV.36: record the exact transform just consumed -- odom_pose is
  // the actual tf_buffer_->transform() output used below to populate
  // x/y/yaw, not a second lookup.
  if (diag_recorder_.enabled()) {
    coco_nav_diag::OdomTfLookupEvent ev;
    ev.update_index = diag_update_index_;
    diagSplitStamp(sensor_timestamp, ev.scan_stamp_sec, ev.scan_stamp_nanosec);
    ...
    ev.odom_y = y;
    ev.odom_yaw = yaw;
    ev.odom_qx = odom_pose.pose.orientation.x;
    ev.odom_qy = odom_pose.pose.orientation.y;
    ev.odom_qz = odom_pose.pose.orientation.z;
    ev.odom_qw = odom_pose.pose.orientation.w;
    ev.odom_pose_stamp_sec = odom_pose.header.stamp.sec;
    ev.odom_pose_stamp_nanosec = odom_pose.header.stamp.nanosec;
    diagSplitStamp(now(), ev.node_now_sec, ev.node_now_nanosec);
    diag_recorder_.recordOdomTfLookup(ev);
  }
...
AmclNode::laserReceived(sensor_msgs::msg::LaserScan::ConstSharedPtr laser_scan)
{
  ...
  // C2-NAV.36: one update_index per laserReceived() call that reaches the
  // odometry lookup below, whether or not that lookup succeeds. Read
  // directly by getOdomPose() (same object) so its call signature does not
  // change.
  ++diag_update_index_;
  ...
  if (!pf_init_) {
    // C2-NAV.36: anchor-initialization event. No delta/motion-model call
    // happens this cycle; pf_odom_pose_ (the anchor) is about to be SET,
    // not differenced against. Logged before the assignment below so
    // anchor_valid=false correctly reflects "no prior anchor consumed".
    if (diag_recorder_.enabled()) {
      coco_nav_diag::MotionDeltaEvent ev;
      ev.update_index = diag_update_index_;
      diagSplitStamp(
        rclcpp::Time(laser_scan->header.stamp), ev.scan_stamp_sec,
        ev.scan_stamp_nanosec);
      ev.is_anchor_init = true;
      ev.anchor_valid = false;
      ev.pose_x = pose.v[0];
      ev.pose_y = pose.v[1];
      ev.pose_yaw = pose.v[2];
      ev.motion_model_type = robot_model_type_;
      ev.should_update_filter = false;
      ev.motion_update_invoked = false;
      diagSplitStamp(now(), ev.node_now_sec, ev.node_now_nanosec);
    ...
    pf_odom_pose_ = pose;
    // C2-NAV.36: this cycle is what will be the anchor for the next one.
    diag_anchor_update_index_ = diag_update_index_;
    pf_init_ = true;
    ...
    bool should_update_filter = shouldUpdateFilter(pose, delta);
    if (should_update_filter) {
      for (unsigned int i = 0; i < lasers_update_.size(); i++) {
        lasers_update_[i] = true;
      }
    }
    // C2-NAV.36: snapshot before the possible motion-model call below.
    // Semantically identical to the original `if (lasers_update_[laser_index])`
    // check -- nothing mutates lasers_update_[laser_index] between this
    // snapshot and its use, so this is a pure refactor, not a behavior
    // change.
    bool motion_update_invoked = lasers_update_[laser_index];

    // C2-NAV.36: motion-delta event, at the exact call site of the
    // motion-model update this task exists to observe. pf_odom_pose_ here
    // is still the pre-update anchor: it is only advanced later, inside
    // updateFilter(), which has not run yet on this path. delta is the
    // shouldUpdateFilter() out-param, exactly what would be (or is) passed
    // to motion_model_->odometryUpdate() below. delta_rot1/delta_trans/
    // delta_rot2 are computed with nav2_amcl::angleutils::angle_diff, the
    // same header function nav2_amcl::DifferentialMotionModel::
    // odometryUpdate() uses on the same inputs (old_pose.v[2] there equals
    // pf_odom_pose_.v[2] here, since old_pose = pose - delta = anchor) --
    // this is not a re-derivation with a different formula, it is the
    // identical inline function applied to values proven equal to what the
    // real (unmodified) motion-model library receives.
    if (diag_recorder_.enabled()) {
      coco_nav_diag::MotionDeltaEvent ev;
      ev.update_index = diag_update_index_;
      diagSplitStamp(
        rclcpp::Time(laser_scan->header.stamp), ev.scan_stamp_sec,
        ev.scan_stamp_nanosec);
      ev.is_anchor_init = false;
      ev.anchor_valid = true;
      ev.anchor_update_index = diag_anchor_update_index_;
      ev.anchor_x = pf_odom_pose_.v[0];
      ev.anchor_y = pf_odom_pose_.v[1];
      ev.anchor_yaw = pf_odom_pose_.v[2];
      ev.pose_x = pose.v[0];
      ev.pose_y = pose.v[1];
      ev.pose_yaw = pose.v[2];
      ev.delta_x = delta.v[0];
      ev.delta_y = delta.v[1];
      ev.delta_yaw = delta.v[2];
      ev.motion_model_type = robot_model_type_;
      ev.should_update_filter = should_update_filter;
  ...
  const pf_vector_t & pose)
{
  ...
  pf_odom_pose_ = pose;
  // C2-NAV.36: this cycle's update_index becomes the anchor for whichever
  // future cycle next differences against the just-advanced pf_odom_pose_.
  diag_anchor_update_index_ = diag_update_index_;
  return true;
}
...
AmclNode::initParameters()
{
  ...
  // C2-NAV.36: read the diag_* parameters and configure diag_recorder_.
  // configure() is a one-shot: initParameters() only runs once per
  // configure() lifecycle transition in this node's actual call graph
  // (single call site above), and DiagRecorder::configure() itself no-ops
  // (with a logged warning) on a second call rather than reopening the
  // output file, so a cleanup/configure lifecycle cycle does not attempt
  // to re-arm capture. This is a known limitation, not a silent gap: see
  // docs/agents/HANDOFF.md.
  bool diag_enabled = false;
  std::string diag_output_path;
  int diag_max_events = 200000;
  get_parameter("diag_enabled", diag_enabled);
  get_parameter("diag_output_path", diag_output_path);
  get_parameter("diag_max_events", diag_max_events);
  ...
  if (diag_recorder_.enabled()) {
    RCLCPP_WARN(
      get_logger(),
      "C2-NAV.36 diagnostic capture ENABLED, writing to '%s' (max_events=%d). "
      "This does not change AMCL's own behavior, only observes it.",
      diag_output_path.c_str(), diag_max_events);
  }
...
// C2-NAV.36: class renamed from nav2_amcl::AmclNode to coco_nav_diag::AmclNode.
RCLCPP_COMPONENTS_REGISTER_NODE(coco_nav_diag::AmclNode)
