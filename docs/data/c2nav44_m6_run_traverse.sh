#!/usr/bin/env bash
# C2-NAV.44 variant: the HISTORICAL M6 harness on the fixed command path.
# mission.launch.py executive:=false + traverse_demo.py --colour <c>, which is
# the sequencer the 19/20 fetch matrix was measured with (docs/RUNNING.md,
# "The sequencer, and the script it replaced"). Same simulator, same nav2
# config, same command path, same recorders. The executive's PRE_RAMP region
# gate does not exist in this harness -- that is the point of running it.
...
setsid ros2 launch coco_mission mission.launch.py rviz:=false executive:=false \
    target_colour:="$COLOUR" > "$OUT/mission.log" 2>&1 &
PGIDS+=("$!")
...
for n in /localization_monitor /mission_hud /ramp_driver \
...
python3 -P "$WT/docs/data/c2nav42_cmdpath.py" record --out "$OUT/cmdpath" \
    --duration "$MISSION_BUDGET" > "$OUT/cmdpath.log" 2>&1 &
REC_PIDS+=("$!")
CMDPATH_PID=$!
python3 -P "$WT/docs/data/c2m51_hrec.py" --out "$OUT/hrec.csv" --tag c2nav44_trav \
    > "$OUT/hrec.log" 2>&1 &
REC_PIDS+=("$!")
python3 -P "$WT/docs/data/c2nav44_poserec.py" \
    --out "$OUT/poses.csv" --duration "$MISSION_BUDGET" > "$OUT/poserec.log" 2>&1 &
REC_PIDS+=("$!")
ros2 topic echo /mission/state --field data > "$OUT/state_stream.txt" 2>&1 &
REC_PIDS+=("$!")
sleep 8

say "starting the HISTORICAL harness: traverse_demo.py --colour $COLOUR"
START_S=$SECONDS
timeout 900 ros2 run gazebo_models traverse_demo.py --colour "$COLOUR" \
    > "$OUT/traverse.log" 2>&1
TRAV_RC=$?
say "traverse_demo rc=$TRAV_RC after $((SECONDS - START_S)) wall s"
echo "traverse_rc=$TRAV_RC" >> "$OUT/meta.txt"
tail -25 "$OUT/traverse.log"
CHECK="traverse_demo reports FETCH COMPLETE"
check grep -q 'FETCH COMPLETE' "$OUT/traverse.log"
timeout 12 ros2 topic echo /amcl_pose --once > "$OUT/final_amcl_pose.txt" 2>&1
sleep 3
kill -INT "$CMDPATH_PID" 2>/dev/null
wait "$CMDPATH_PID"
tail -60 "$OUT/cmdpath.log"
