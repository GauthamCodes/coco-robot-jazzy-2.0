...
T0 = time.time()
_FH = open(f'{TMP}/isaac_stack4.txt', 'w')
faulthandler.dump_traceback_later(420, repeat=False, file=_FH)
...
try:
    ...
    for _ in range(10):
        app.update()
    stamp('BRIDGE extension enabled')
    ...
    stamp('PUBLISHING for 90 s wall')
    end = time.time() + 90
    updates = 0
    t = time.time()
    while time.time() < end:
        world.step(render=True)      # app.update(): ticks the graph
