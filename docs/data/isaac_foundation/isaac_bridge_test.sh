...
env -i HOME=/home/gautham PATH=/usr/bin:/bin \
    ...
    timeout 600 "$VENV/bin/python" -u "$TMP/isaac_smoke4.py" \
    >> "$TMP/isaac_smoke4.log" 2>&1 &
ISAAC=$!
...
(
  ...
  timeout 30 ros2 topic echo /clock rosgraph_msgs/msg/Clock --once \
    > "$TMP/jazzy_clock.txt" 2>&1; echo "clock echo exit=$?"
