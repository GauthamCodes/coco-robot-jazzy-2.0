...
# The instrumented AMCL (coco_nav_diag) and its GT sidecar live on the C2-NAV
# diagnosis branch only. Refuse before a simulator starts rather than at step 6.
if [ "$DIAG" = true ] && [ ! -f "$WT/coco_nav_diag/scripts/amcl_diag_swap.py" ]; then
    die "amcl_diag.enabled needs coco_nav_diag, which this tree does not carry"
fi
