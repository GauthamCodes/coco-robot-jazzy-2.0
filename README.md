...
* **The collision monitor's authority over the wheels was restored late, and
  the fetch result predates it.** Until C2-NAV.42 the arbiter read
  `/cmd_vel_nav`, the controller's raw command, so the monitor's gating did
  not reach the wheels. The relay now publishes `/cmd_vel_gated` and the
  arbiter reads that. Measured on the fixed path: raw-controller bypass rows
  349 → 0 over three tours, and a held raw 0.30 m/s command stopped by
  PolygonStop 0.249 m from a wall. The standing 19/20 was measured with the
  loop in place and has **not** been re-run. A residual remains in the tour
  trace (wheels above the monitor on 2.46 % of samples), attributed to the
  arbiter re-sending gated commands up to 0.078 s old. Details:
  `docs/agents/C2-NAV.42_RESULTS.md`.
